...
bool build_cap_table(COptions& options, int argc, const char *argv[]) {
    ...
    do {
        ...
        if (blockNum >= options.start) {
            ...
            if (true) { //}!trans.isError) { // ignore errors
                CBlock block;
                ...
                for (auto log : trans.receipt.logs) {
                    ...
                    if (log.articulatedLog.name == "CreatedToken" ||
                        log.articulatedLog.name == "Mint") {
                        ...
                        if (verbose) {
                            ...
                            replace(result, "{DATE}",   substitute(substitute(trans.Format("[{DATE}]"), " UTC", "")," ","T"));
...
//-------------------------------------------------------------------------
const string_q fmtOut = "{TYPE}\t[{TIMESTAMP}]\t[{BLOCKNUMBER}].[{TRANSACTIONINDEX}]\t[{ISERROR}]\t{HOLDER}\t[{r:28:VALUE}]\t{BAL1}\t{CHNG}\t{BAL2}";
...
//#define EXPORT
    ...
    string_q fmt = "Block:   [{BN}]  Date:   [{DATE}]\n"
